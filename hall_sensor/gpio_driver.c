...
static ktime_t time1 = 0;
static ktime_t time2 = 0;
unsigned int time_passed;
//static int dozvola = 0;
...
static ssize_t gpio_driver_read(struct file *filp, char *buf, size_t len, loff_t *f_pos)
{
    ...
    int data_size = 0, duzina;

    /* TODO: fill gpio_driver_buffer here. */
    memset(gpio_driver_buffer, 0, BUF_LEN);
    
    while(GetGpioPinValue(GPIO_17) == 0){
        //dozvola = 0;
    }
    
    //if(GetGpioPinValue(GPIO_17) == 1 && dozvola == 0){
        time2 = ktime_get();
        
       // dozvola = 1;
        
        time_passed = ktime_sub(time2, time1);
        
        printk(KERN_DEBUG "Vreme: %u\n", time_passed);
        
        time1 = time2;
        
        duzina = sprintf(gpio_driver_buffer, "%u", time_passed);
        
        gpio_driver_buffer[duzina-1] = '\n';
    //}
